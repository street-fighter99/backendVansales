...
public interface PurchaseRepo extends JpaRepository<PurchaseEntity,Integer> {
    ...
    @Query(value = "UPDATE `vansale`.`purchase` SET purchase_id = ?1,supplier_id = ?2,item_list=?3,total_amount = ?4,discount = ?5,aftdiscount=?6,net_amount = ?7,paid_amount = ?8,balance = ?9,total_balance = ?10,total_balance = ?11,p_date = ?12,user_id = ?13  WHERE (`user_id` = ?13 ) and (`purchase_id` = ?1)",nativeQuery = true)
